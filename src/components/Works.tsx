...
const Works = () => {
  ...
  return (
    <div id='Works' className='w-full min-h-screen bg-gradient-to-b from-black to-gray-800 p-4 text-white'>
      <div className='max-w-screen-lg mx-auto p-4 text-white'>
        <div>
          <p className='text-4xl font-bold border-b-4 border-gray-500 p-2 inline'>Works</p>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 text-center py-8 px-4 sm:px-12'>
          {WorkItems.map((item, index) => (
            <div key={index} className='shadow-md shadow-gray-600 hover:scale-105 duration-500 py-2 rounded-lg bg-gray-800'>
              <img src={item.imageUrl} alt={item.title} className='w-60 h-auto mx-auto rounded-md' />
